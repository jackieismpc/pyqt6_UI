...
原始输入面板只显示预览：静态图片通过 `QImageReader` 限制最长边为 1280 px 后再交给 Qt，视频预览最多 15 FPS、最长边 1280 px；不会把 4K/8K 原图作为长期 `QPixmap` 保留。实时摄像头预览仍限制为约 640 px，算法拍摄帧由后端单独处理。
...
一次性任务线程结束后会调用 `deleteLater` 并清除引用；视频面板切换到静态图或关闭窗口时会停止 `QTimer` 并释放 `VideoCapture`。未勾选保存的临时输出会在下一次任务或程序退出时清理。
